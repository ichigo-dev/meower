    ...
    C: ConnectionTrait,
{
    ...
    if input.password != input.password_confirm
    {
        return Err
        (
            i18n.get("entrance.signup.form.error.password_not_match")
        );
    }
            ...
            .unwrap()
    {
        ...
        None =>
        {
            let error = i18n.get
            (
                "entrance.forgot_password.form.error.token_not_found"
            );
            return Err(error);
        },
    };
    ...
    let user = match UserEntity::find_by_id(user_id).one(hdb).await.unwrap()
    {
        ...
        None =>
        {
            let error = i18n.get
            (
                "entrance.forgot_password.form.error.email_not_found"
            );
            return Err(error);
        },
    };
