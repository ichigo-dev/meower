...
impl I18n
{
    ...
    pub fn init( &mut self, locale: &str, config: &Config ) -> bool
    {
        ...
        let mut locale_path = PathBuf::from(&config.get("locale.path"));
        locale_path.push(&self.locale);
        self.map = self.load(&locale_path);

        let mut locale_path = PathBuf::from(&config.get("locale.path"));
        locale_path.push(&config.get("locale.default"));
    ...
    fn find_locale_path( &mut self, config: &Config ) -> bool
    {
        ...
        {
            let mut fallback_path = PathBuf::from(&config.get("locale.path"));
            fallback_path.push(&config.get("locale.default"));
            if fallback_path.exists() == false
            {
                return false;
            }
            self.locale = config.get("locale.default").to_string();
        }
        true
    }
