...
#[sea_orm(table_name = "group_cover")]
pub struct Model
{
    #[sea_orm(primary_key)]
    pub group_cover_id: i64,
    pub group_id: i64,
    #[sea_orm(unique)]
    pub file_key: String,
...
impl Column
{
    ...
    pub fn get_name( &self ) -> String
    {
        match self
        {
            Self::GroupCoverId => t!("entities.group_cover.group_cover_id.name"),
            Self::GroupId => t!("entities.group_cover.group_id.name"),
            Self::FileKey => t!("entities.group_cover.file_key.name"),
            Self::FileName => t!("entities.group_cover.file_name.name"),
            Self::FileSize => t!("entities.group_cover.file_size.name"),
            Self::ContentType => t!("entities.group_cover.content_type.name"),
            Self::CreatedAt => t!("entities.group_cover.created_at.name"),
        }
    }
}
...
pub enum Error
{
    #[error("GroupCover: {column:?} {error:?}")]
    Validation { column: Column, error: ValidationError },

    #[error("GroupCover: Database error.")]
    DbError(#[from] DbErr),
}
