...
#[sea_orm(table_name = "group_member_request")]
pub struct Model
{
    #[sea_orm(primary_key)]
    pub group_member_request_id: i64,
    pub group_id: i64,
    pub account_id: i64,
    pub account_profile_id: i64,
}

#[Object(name = "GroupMemberRequest")]
impl Model
{
    //--------------------------------------------------------------------------
    /// Gets the group.
...
impl Column
{
    ...
    pub fn get_name( &self ) -> String
    {
        match self
        {
            Self::GroupMemberRequestId => t!("entities.group_member_request.group_member_request_id.name"),
            Self::GroupId => t!("entities.group_member_request.group_id.name"),
            Self::AccountId => t!("entities.group_member_request.account_id.name"),
            Self::AccountProfileId => t!("entities.group_member_request.account_profile_id.name"),
        }
    }
}
...
pub enum Error
{
    #[error("GroupMemberRequest: {column:?} {error:?}")]
    Validation { column: Column, error: ValidationError },

    #[error("GroupMemberRequest: Database error.")]
    DbError(#[from] DbErr),
}
